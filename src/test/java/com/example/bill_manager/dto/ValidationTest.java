...
class ValidationTest {
  ...
  class LineItemValidation {
    ...
    void shouldPassForValidLineItem() {
      LineItem item =
          new LineItem("Milk", new BigDecimal("2"), new BigDecimal("3.49"), new BigDecimal("6.98"));
    ...
    void shouldAllowZeroPrices() {
      LineItem item =
          new LineItem("Free Item", new BigDecimal("1"), BigDecimal.ZERO, BigDecimal.ZERO);
    ...
    void shouldRejectBlankName() {
      LineItem item =
          new LineItem("", new BigDecimal("1"), new BigDecimal("1.00"), new BigDecimal("1.00"));
    ...
    void shouldRejectNegativeUnitPrice() {
      LineItem item =
          new LineItem(
              "Item", new BigDecimal("1"), new BigDecimal("-1.00"), new BigDecimal("1.00"));
    ...
    void shouldRejectZeroQuantity() {
      LineItem item =
          new LineItem("Item", BigDecimal.ZERO, new BigDecimal("1.00"), new BigDecimal("0.00"));
  ...
  class BillAnalysisResultValidation {
    ...
    void shouldPassForValidResult() {
      ...
      Set<ConstraintViolation<BillAnalysisResult>> violations = validator.validate(result);
    ...
    void shouldRejectNegativeTotalAmount() {
      BillAnalysisResult result =
          new BillAnalysisResult(
              "Store", List.of(createValidLineItem()), new BigDecimal("-10.00"), "PLN", List.of());
    ...
    void shouldRejectBlankMerchantName() {
      BillAnalysisResult result =
          new BillAnalysisResult(
              "", List.of(createValidLineItem()), new BigDecimal("10.00"), "PLN", List.of());

      Set<ConstraintViolation<BillAnalysisResult>> violations = validator.validate(result);

      assertThat(violations).isNotEmpty();
      assertThat(violations).anyMatch(v -> v.getPropertyPath().toString().equals("merchantName"));
    }

    @Test
    void shouldRejectEmptyItems() {
      BillAnalysisResult result =
          new BillAnalysisResult("Store", List.of(), new BigDecimal("10.00"), "PLN", List.of());
    ...
    void shouldCascadeValidationToLineItems() {
      ...
      BillAnalysisResult result =
          new BillAnalysisResult(
              "Store", List.of(invalidItem), new BigDecimal("10.00"), "PLN", List.of());
    ...
    void shouldAllowNullCategoryTags() {
      BillAnalysisResult result =
          new BillAnalysisResult(
              "Store", List.of(createValidLineItem()), new BigDecimal("10.00"), "PLN", null);
  ...
  class BillAnalysisResponseValidation {
    ...
    void shouldPassForValidResponse() {
      BillAnalysisResponse response =
          new BillAnalysisResponse(
              UUID.randomUUID(), "receipt.jpg", createValidResult(), Instant.now());

      Set<ConstraintViolation<BillAnalysisResponse>> violations = validator.validate(response);
    ...
    void shouldRejectNullId() {
      BillAnalysisResponse response =
          new BillAnalysisResponse(null, "receipt.jpg", createValidResult(), Instant.now());
    ...
    void shouldRejectBlankFileName() {
      BillAnalysisResponse response =
          new BillAnalysisResponse(UUID.randomUUID(), "", createValidResult(), Instant.now());
  ...
  private static LineItem createValidLineItem() {
    return new LineItem(
        "Item", new BigDecimal("1"), new BigDecimal("9.99"), new BigDecimal("9.99"));
  }
